...
from app.services import user_preferences as services

router = APIRouter(prefix="/preferences", tags=["preferences"])


@router.get("", response_model=Envelope[schemas.Preferences],
...
def get_preferences(user_id: Annotated[str, Depends(get_current_user_id)]):
    ...
    """
    row = services.get_preferences(user_id=user_id)
    return envelope_(data=row)


@router.patch("", response_model=Envelope[schemas.Preferences],
              responses={401: {"model": ErrorEnvelope}})
def update_preferences(
    ...
    row = services.update_preferences(user_id=user_id, fields=fields)
    return envelope_(data=row)
